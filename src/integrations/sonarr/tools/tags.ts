...
export const tool: ToolDefinition = {
  ...
  integration: 'sonarr',
  description:
    'List, create, or delete tags in Sonarr. Tags can be used to organize series and apply settings to groups.',
  parameters: {
    type: 'object',
    properties: {
      action: {
  ...
  async handler(params, ctx) {
    ...
    // List
    ctx.log('Fetching Sonarr tags...');
    const tags: any[] = await client.get('/api/v3/tag');

    if (!Array.isArray(tags) || tags.length === 0) {
      return {
        success: true,
        message: 'No tags configured in Sonarr.',
        data: { tags: [] },
      };
    }
