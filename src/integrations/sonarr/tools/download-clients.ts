...
export const tool: ToolDefinition = {
  name: 'sonarr_download_clients',
  integration: 'sonarr',
  description:
    'List all download clients configured in Sonarr with their status and settings.',
  parameters: {
    type: 'object',
    properties: {},
  },
  ...
  async handler(_params, ctx) {
    ...
    ctx.log('Fetching Sonarr download clients...');
    ...
    if (!Array.isArray(clients) || clients.length === 0) {
      return {
        success: true,
        message: 'No download clients configured in Sonarr.',
