...
export const tool: ToolDefinition = {
  name: 'radarr_blocklist',
  integration: 'radarr',
  description:
    'View or manage the Radarr blocklist. Shows releases that have been blocklisted to prevent re-downloading. Can also remove items from the blocklist.',
  parameters: {
    type: 'object',
    properties: {
      action: {
  ...
  async handler(params, ctx) {
    ...
    const lines = items.map(
      (item: any) =>
        `- [ID: ${item.id}] ${item.movieTitle} — "${item.sourceTitle}" (${item.quality}, ${item.indexer}, ${item.date})`,
    );
