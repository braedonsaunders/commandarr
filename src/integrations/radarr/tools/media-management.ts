...
export const tool: ToolDefinition = {
  name: 'radarr_media_management',
  integration: 'radarr',
  description:
    'View or update Radarr media management settings. Controls file handling, permissions, recycling bin, importing behavior, and more.',
  parameters: {
    type: 'object',
    properties: {
      action: {
      ...
      autoUnmonitorPreviouslyDownloadedMovies: {
        type: 'boolean',
        description: 'Automatically unmonitor movies after download (for "update")',
      },
  ...
  async handler(params, ctx) {
    ...
    if (action === 'get') {
      ctx.log('Fetching Radarr media management config...');
      const config = await client.get('/api/v3/config/mediamanagement');

      return {
        success: true,
        message: [
          'Radarr Media Management Settings:',
          `Recycle Bin: ${config.recycleBin || '(not set)'}`,
          `Recycle Bin Cleanup: ${config.recycleBinCleanupDays} days`,
          `Auto Unmonitor Downloaded: ${config.autoUnmonitorPreviouslyDownloadedMovies ? 'Yes' : 'No'}`,
          ...
          `Create Empty Folders: ${config.createEmptyMovieFolders ? 'Yes' : 'No'}`,
    ...
    for (const key of [
      'recycleBin', 'recycleBinCleanupDays', 'autoUnmonitorPreviouslyDownloadedMovies',
